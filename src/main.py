...
start_time = datetime(2022, 11, 4, 3, 0)
...
coords = location_data.Geolocation.str.strip('{}').str.split(expand=True).astype(float)
opening_times = location_data["Opening Times"].str.split('-', expand=True).astype(int)
coords = pd.concat([coords, location_data["Building Name"], opening_times], axis=1)
...
joined = security_log_data.join(location_data.set_index('Building Name'), on='Location')

joined = joined[["Name", "Time", "Geolocation"]]

joined[['start', 'end']] = joined.Time.str.split('-', expand=True).astype(int)
joined[['lat', 'lon']] = joined.Geolocation.str.strip('{}').str.split(expand=True).astype(float)
joined = joined.drop(columns = ['Time', 'Geolocation'])

timelines = {}
for _, person in people_data.iterrows():
    timelines[person['Name']] = joined.loc[joined["Name"] == person["Name"]].drop(columns=['Name'])
...
def curr_loc(name, time):
    ...
    last_lat = -55
    last_lon = 176
    last_endtime = datetime(2022, 11, 4, 0, 0)
    for _, loc in timeline.iterrows():
        start, end, lat, lon = loc
        
        start = datetime(2022, 11, 4, int(start)//100, int(start)%100)
        end = datetime(2022, 11, 4, int(end)//100, int(end)%100)
